...
- `iop/demosaic.c`: no scaling. `modify_roi_out()` snaps `x,y` to the mosaic origin (currently `0,0`). `modify_roi_in()` does not snap or align `x,y` — any crop offset is accepted as-is; `process()`/`process_cl()` compute the correct Bayer/X-Trans phase for that exact offset on every call instead (see CLAUDE.md, "Demosaic's Bayer/X-Trans phase is computed fresh per crop, not snapped").
